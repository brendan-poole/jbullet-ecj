...
public class PhysicsModel extends DemoApplication {
	...
	public void initPhysics() {
		...
		{
			CollisionShape groundShape = new BoxShape(new Vector3f(1000f, 10f,
					1000f));
		...
		for (int i = 0; i < 10; i++) {
			...
			startTransform.origin.set(0, -2.5f, 50 - i * 5);
			DefaultMotionState myMotionState = new DefaultMotionState(
					startTransform);
			RigidBodyConstructionInfo rbInfo = new RigidBodyConstructionInfo(
					mass, myMotionState, colShape, localInertia);
			bod[i] = new RigidBody(rbInfo);
			dynamicsWorld.addRigidBody(bod[i]);

		}
		/*
		 * { float mass = 1f; Vector3f localInertia = new Vector3f(0, 0, 0);
		 * colShape.calculateLocalInertia(mass, localInertia);
		 * startTransform.setIdentity(); startTransform.origin.set(2, 0, 0);
		 * DefaultMotionState myMotionState = new
		 * DefaultMotionState(startTransform); RigidBodyConstructionInfo rbInfo
		 * = new RigidBodyConstructionInfo(mass, myMotionState, colShape,
		 * localInertia); bod2 = new RigidBody(rbInfo);
		 * dynamicsWorld.addRigidBody(bod2); } { float mass = 1f; Vector3f
		 * localInertia = new Vector3f(0, 0, 0);
		 * colShape.calculateLocalInertia(mass, localInertia);
		 * startTransform.setIdentity(); startTransform.origin.set(4, 0, 0);
		 * DefaultMotionState myMotionState = new
		 * DefaultMotionState(startTransform); RigidBodyConstructionInfo rbInfo
		 * = new RigidBodyConstructionInfo(mass, myMotionState, colShape,
		 * localInertia); bod3 = new RigidBody(rbInfo);
		 * dynamicsWorld.addRigidBody(bod3); } {(float) ( float mass = 1f;
		 * Vector3f localInertia = new Vector3f(0, 0, 0);
		 * colShape.calculateLocalInertia(mass, localInertia);
		 * startTransform.setIdentity(); startTransform.origin.set(6f, 0, 0);
		 * DefaultMotionState myMotionState = new
		 * DefaultMotionState(startTransform); RigidBodyConstructionInfo rbInfo
		 * = new RigidBodyConstructionInfo(mass, myMotionState, colShape,
		 * localInertia); bod4 = new RigidBody(rbInfo);
		 * dynamicsWorld.addRigidBody(bod4); } { float mass = 1f; Vector3f
		 * localInertia = new Vector3f(0, 0, 0);
		 * colShape.calculateLocalInertia(mass, localInertia);
		 * startTransform.setIdentity(); startTransform.origin.set(8f, 0, 0);
		 * DefaultMotionState myMotionState = new
		 * DefaultMotionState(startTransform); RigidBodyConstructionInfo rbInfo
		 * = new RigidBodyConstructionInfo(mass, myMotionState, colShape,
		 * localInertia); bod5 = new RigidBody(rbImotorsnfo);
		 ...
		 * localB.origin.set(0.0f, 0.0f, 1.0f); liftHinge = new
		 * HingeConstraint(bod, bod2, localA, localB); liftHinge.setLimit(-PI /
		 * 2f, PI / 2f); dynamicsWorld.addConstraint(liftHinge, true); } {
		 * Transform localA = new Transform(), localB = new Transform();
		 * localA.setIdentity(); localB.setIdentity();
		 * MatrixUtil.setEulerZYX(localA.basis, 0, PI_2, 0);
		 * localA.origin.set(0.0f, 0.0f, -1.0f);
		 * MatrixUtil.setEulerZYX(localB.basis, 0, PI_2, 0);
		 * localB.origin.set(0.0f, 0.0f, 1.0f); liftHinge2 = new
		 * HingeConstraint(bod2, bod3, localA, localB); liftHinge2.setLimit(-PI
		 * / 2f, PI / 2f); dynamicsWorld.addConstraint(liftHinge2, true); } {
		 * Transform localA = new Transform(), localB = new Transform();
		 * localA.setIdentity(); localB.setIdentity();
		 * MatrixUtil.setEulerZYX(localA.basis, 0, PI_2, 0);
		 * localA.origin.set(0.0f, 0.0f, -1.0f);
		 * MatrixUtil.setEulerZYX(localB.basis, 0, PI_2, 0);
		 * localB.origin.set(0.0f, 0.0f, 1.0f); liftHinge3 = new
		 * HingeConstraint(bod3, bod4, localA, localB); liftHinge3.setLimit(-PI
		 * / 2f, PI / 2f); dynamicsWorld.addConstraint(liftHinge3, true);
		 * 
		 * 
		 * } { Transform localA = new Transform(), localB = new Transform();
		 * localA.setIdentity(); localB.setIdentity();
		 ...
		 * localA.origin.set(0.0f, 0.0f, -1.0f);
		 * MatrixUtil.setEulerZYX(localB.basis, 0, PI_2, 0);
		 * localB.origin.set(0.0f, 0.0f, 1.0f); liftHinge4 = new
		 * HingeConstraint(bod4, bod5, localA, localB); liftHinge4.setLimit(-PI
		 * / 2f, PI / 2f); dynamicsWorld.addConstraint(liftHinge4, true);
		 * 
		 * 
		 * }
		 */
		/*
		 * Transform localA = new Transform(), localB = new Transform();
		 * localA.setIdentity(); localB.setIdentity(); localA.origin.set(2f, 0f,
		 * 0f); localB.origin.set(4f, 0f, 0f); ConeTwistConstraint c = new
		 * ConeTwistConstraint(bod,bod2,localA,localB); c.setLimit(.5f,.5f,
		 * .5f); dynamicsWorld.addConstraint(c,true);
		 */
		/*
		 * { Point2PointConstraint p = new Point2PointConstraint(bod,bod2,new
		 * Vector3f(2.5f,0,0),new Vector3f(-2,0,0));
		 * dynamicsWorld.addConstraint(p, false); }
		 */
		/*
		 * { Point2PointConstraint p = new Point2PointConstraint(bod2,bod3,new
		 * Vector3f(0,0,0),new Vector3f(2,0,0)); dynamicsWorld.addConstraint(p,
		 * true); }
		 */

		/*
		 * Transform localA = new Transform(), localB = new Transform(); boolean
		 * useLinearReferenceFrameA = true; localA.setIdentity();
		 * localB.setIdentity(); localA.origin.set(2f, 0f, 0f);
		 * localB.origin.set(4f, 0f, 0f);
		 * 
		 * Generic6DofConstraint joint6DOF; joint6DOF = new
		 * Generic6DofConstraint(bod2, bod3, localA, localB,
		 * useLinearReferenceFrameA); Vector3f tmp = new Vector3f(0,0,0);
		 * //tmp.set(-BulletGlobals.SIMD_PI * 0.3f, -BulletGlobals.FLT_EPSILON,
		 * -BulletGlobals.SIMD_PI * 0.3f); joint6DOF.setAngularLowerLimit(tmp);
		 * //tmp.set(BulletGlobals.SIMD_PI * 0.5f, BulletGlobals.FLT_EPSILON,
		 * BulletGlobals.SIMD_PI * 0.3f); joint6DOF.setAngularUpperLimit(tmp);
		 * 
	...
	public void spawnRagdoll(boolean random) {
		RagDoll ragDoll = new RagDoll(dynamicsWorld, new Vector3f(0f, 15f, 0f),
				3f);
	...
	public void clientMoveAndDisplay() {
		...
		for (int i = 0; i < 30; i++)
			this.ragdolls.get(0).motors[i].targetVelocity = inputs[i]
					.getValue();
		...
		for (int i = 0; i < n; i++) {
			PersistentManifold cm = dynamicsWorld.getDispatcher()
	...
	public void keyboardMouse() {
		...
		while (Keyboard.next()) {
			if (Keyboard.getEventCharacter() != '\0') {
				keyboardCallback(Keyboard.getEventCharacter(), Mouse.getX(),
						Mouse.getY(), modifiers);
			}

			if (Keyboard.getEventKeyState()) {
				specialKeyboard(Keyboard.getEventKey(), Mouse.getX(),
						Mouse.getY(), modifiers);
			} else {
				specialKeyboardUp(Keyboard.getEventKey(), Mouse.getX(),
						Mouse.getY(), modifiers);
			}
		...
		while (Mouse.next()) {
			if (Mouse.getEventButton() != -1) {
				...
				mouseFunc(btn, Mouse.getEventButtonState() ? 0 : 1,
						Mouse.getEventX(), Display.getDisplayMode().getHeight()
								- 1 - Mouse.getEventY());
			}
			mouseMotionFunc(Mouse.getEventX(), Display.getDisplayMode()
					.getHeight() - 1 - Mouse.getEventY());
		}
